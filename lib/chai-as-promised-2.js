        ...
        Assertion.overwriteMethod(methodName, function (originalMethod) {
            return function () {
                ...
                var derivedPromise = that._obj.then(function (value) {
                    that._obj = value;
                    utils.flag(that, "eventually", false);
        ...
        if (isChainableMethod) {
            Assertion.addChainableMethod(
                ...
                function () {
                    ...
                    if (!utils.flag(that, "eventually")) {
                        return propertyDesc.get.call(that).apply(that, args);
                    }

                    var derivedPromise = that._obj.then(function (value) {
                        that._obj = value;
                        utils.flag(that, "eventually", false);
                        propertyDesc.get.call(that).apply(that, args);
                    });
                ...
                function () {
                    ...
                    if (!utils.flag(that, "eventually")) {
                        return propertyDesc.get.call(that);
                    }

                    var derivedPromise = that._obj.then(function (value) {
                        that._obj = value;
                        utils.flag(that, "eventually", false);
                        propertyDesc.get.call(that);
                    });
            ...
            Assertion.overwriteProperty(getterName, function (originalGetter) {
                return function () {
                    ...
                    if (!utils.flag(that, "eventually")) {
                        return originalGetter.apply(that);
                    }

                    var derivedPromise = that._obj.then(function (value) {
                        that._obj = value;
                        utils.flag(that, "eventually", false);
                        originalGetter.apply(that);
                    });
